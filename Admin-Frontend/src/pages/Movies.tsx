...
const Movies = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${backend_url}/movie/${groupId}/${deleteMovieId}`);
      alert("Movie removed successfully!");
      fetchCurrentMovies();
      setShowConfirm(false);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white p-6 flex flex-col items-center space-y-10">
        ...
        {movies.length > 0 ? (
          movies.map((movie, index) => (
            <div
              ...
              } w-full sm:w-[48%] md:w-[30%] lg:w-[22%] rounded-2xl shadow-lg overflow-hidden transition-transform transform hover:scale-105`}
            >
              <img
                src={movie.posters?.[0]}
                alt={movie.title}
      ...
      {/* Add/Edit Movie Modal (Small & Scrollable) */}
      {showForm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111] rounded-2xl max-w-xl w-full max-h-[80vh] overflow-y-auto relative p-6 shadow-lg">
            {/* Close Button */}
            <button
