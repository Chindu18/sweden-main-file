...
const Revenue = () => {
  ...
  const fetchMovies = async () => {
    try {
      const res = await axios.get(`${backendurl}/movie/getmovie`);
        ...
        moviesData.map(async (movie) => {
          try {
            const [pendingResp, paidResp] = await Promise.all([
              axios.get(`${backendurl}/dashboard/pending`, { params: { movieName: movie.title, paymentStatus: "pending" } }),
              axios.get(`${backendurl}/dashboard/pending`, { params: { movieName: movie.title, paymentStatus: "paid" } }),
            ]);
  ...
  const RatioBar = ({ paid, pending }: { paid: number; pending: number }) => {
    ...
    return (
      <div className="w-full overflow-hidden rounded-full border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-800">
        <div className="h-2 bg-emerald-500" style={{ width: `${paidPct}%` }} ></div>
