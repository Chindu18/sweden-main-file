...
interface Booking {
  ...
  email: string;
  phone: string;
  seatNumbers: { row: number; seat: number }[];
  date: string;
  timing: string;
  paymentStatus: string;
  ticketType: string;
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const reversedData = [...data].reverse();
        setAllMovies(reversedData);
        if (reversedData.length > 0) setSelectedMovie(reversedData[0]);
      } catch (err) {
        console.error("Error fetching movies:", err.message);
      }
    };
  ...
  // ---------------- Prepare years and shows for filters ----------------
  const years = ["All", ...Array.from(new Set(allBookings.map(b => new Date(b.date).getFullYear())))];

  const shows = ["All", ...Array.from(new Set(allBookings.map(b => b.date))).sort()];
  ...
  useEffect(() => {
    ...
    if (searchId.trim() !== "")
      filtered = filtered.filter((b) => b.bookingId.toLowerCase().includes(searchId.toLowerCase()));

    if (selectedYear !== "All")
      filtered = filtered.filter((b) => new Date(b.date).getFullYear().toString() === selectedYear.toString());

    if (selectedShow !== "All")
      filtered = filtered.filter((b) => b.date === selectedShow);

    setBookings(filtered);
  }, [paymentStatus, paymentMethod, searchId, allBookings, selectedYear, selectedShow]);
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1">
            ...
            <select
              value={selectedMovie?.title || ""}
              onChange={(e) =>
                setSelectedMovie(allMovies.find((m) => m.title === e.target.value) || null)
              }
              className="w-full border rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-primary outline-none"
            >
              {[...allMovies].reverse().map((m) => (
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">Select Show</label>
           <select
  value={selectedShow}
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <Card>
            <CardContent className="p-6 flex justify-between items-center">
              <div>
                <p className="text-sm text-muted-foreground">Collected</p>
                <p className="text-3xl font-bold mt-2 flex items-center gap-1">SEK {paidMoney}</p>
              </div>
              <TrendingUp className="h-6 w-6 text-green-600" />
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-6 flex justify-between items-center">
              <div>
                <p className="text-sm text-muted-foreground">Pending</p>
                <p className="text-3xl font-bold mt-2 flex items-center gap-1">SEK {pendingMoney}</p>
              </div>
              <Clock className="h-6 w-6 text-orange-600" />
            </CardContent>
          </Card>
